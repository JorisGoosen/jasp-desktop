...
sqlite3_bind_int(stmt,	6, dataSynch);
	...
	transactionWriteBegin();
	int id = runStatementsId("INSERT OR REPLACE INTO DataSets (dataFilePath, dataFileTimestamp, description, databaseJson, emptyValuesJson, dataFileSynch, csvDelimiter, id) VALUES (?, ?, ?, ?, ?, ?, ?, 1) RETURNING id;", prepare);
