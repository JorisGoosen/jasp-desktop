...
std::string Column::labelsTempValue(size_t tempLabelIndex, bool fancyEmptyValue, bool maximumPrecision)
{
	if(labelsTempCount() <= tempLabelIndex)
		return "";
	
	if(tempLabelIndex < _labels.size())
		return _labels[tempLabelIndex]->originalValueAsString(fancyEmptyValue, maximumPrecision);
	
	//So its not from a Label, this means its from _dbls
	//So that means the display value is actually the same as the value so:
	return doubleToDisplayString(_labelsTempDbls[tempLabelIndex], fancyEmptyValue, false, maximumPrecision);
}
