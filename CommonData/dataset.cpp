...
void DataSet::dbCreate()
{
	...
	_dataSetId		= db().dataSetInsert(_dataFilePath, _dataFileTimestamp, _description, _database.toStyledString(), _emptyValues->toJson().toStyledString(), _dataFileSynch);
...
{
	assert(_dataSetId > 0);
	db().dataSetUpdate(_dataSetId, _dataFilePath, _title, _dataFileTimestamp, _description, _database.toStyledString(), _emptyValues->toJson().toStyledString(), _dataFileSynch);
	incRevision();
}
...
void DataSet::dbLoad(int id, std::function<void(float)> progressCallback, Version doUpgradeFrom)
{
	...
	db().dataSetLoad(_dataSetId, _dataFilePath, _title, _dataFileTimestamp, _description, databaseJson, emptyVals, _revision, _dataFileSynch);
