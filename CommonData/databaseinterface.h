...
public:
	...
	void		dataSetUpdate(			int dataSetId,	const std::string & dataFilePath = "", long dataFileTimestamp = 0, const std::string & description = "", const std::string & databaseJson = "", const std::string & emptyValuesJson = "", bool dataSynch = false, bool showRSyntax = false, char csvDelimiter = '\0', bool isComputed = false, const std::string & computeCode = "", const std::string & dependsOn = "");		///< Updates an existing DataSet row in DataSets
	void		dataSetLoad(			int dataSetId,		  std::string & dataFilePath,	long & dataFileTimestamp,		 std::string & description,			   std::string & databaseJson,			  std::string & emptyValuesJson, int & revision, bool & dataSynch, bool & showRSyntax, char & csvDelimiter, bool & isComputed, std::string & computeCode, std::string & dependsOn);	///< Loads an existing DataSet row into arguments
