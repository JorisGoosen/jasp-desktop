...
void FileMenu::sync()
{
	...
	DataSetSyncer * syncer = ws->syncerForDataSet(shown);
	if(syncer)
		syncer->syncNow();
	else if(!MessageForwarder::showYesNo(tr("No associated data file"),
				tr("JASP has no associated data file to be synchronized with.\nDo you want to search for such a data file on your computer?")))
		return;

	QString path = MessageForwarder::browseOpenFile(tr("Find Data File"), "", tr("Data File (*.csv *.txt *.tsv *.sav *.zsav *.ods *.xls *.xlsx *.dta *.por *.sas7bdat *.sas7bcat *.xpt *.rdata *.rds *.mwx *.mpx)"));
	if(!path.isEmpty())
	{
		_mainWindow->setCheckAutomaticSync(false);
		DataSetSyncer * newSyncer = ws->syncerForDataSet(shown);
		if(!newSyncer)
		{
			ws->startSyncForDataSet(shown);
			newSyncer = ws->syncerForDataSet(shown);
		}
		if(newSyncer)
		{
			newSyncer->startFileSyncing(path);
			newSyncer->syncNow();
		}
	}
}
