...
Item
{
	...
	Item
	{
		...
		{
			handleDataAnalyses.x = splitViewContainer.width - (splitViewContainer.hasAnalysis ? handleAnalysesResults.width : 0)
		}
		...
		onHasDataChanged:
		{
			if (hasData && !splitViewContainer.hasAnalysis)	maximizeDataPanel()
		...
		DataPanel
		{
			id:						dataPanel
			visible:				hasData
			width:					splitViewContainer.hasAnalysis ? handleDataAnalyses.x + 1 : parent.width // -1 is for the border to the right
			height:					parent.height
			anchors.left:			engines.right
		}
		...
		JASPSplitHandle
		{
			...
			visible:			hasData && splitViewContainer.hasAnalysis
			onArrowClicked:
			{
				...
				{
					maximizeDataPanel()
					splitViewContainer.keepDataPanelMaximised = true
				}
			}
			pointingLeft:		x > 0
			...
			onDraggingChanged:	{ checkPosition(true); splitViewContainer.keepDataPanelMaximised = false }
			...
			function checkPosition(forceCheck)
			{
				if (forceCheck || !dragging)
				{
					...
					if (x < 0 && !analysesModel.visible && splitViewContainer.hasData)
						x = 0
		...
		Item
		{
			...
			Rectangle
			{
				...
				visible:				!hasData && splitViewContainer.hasAnalysis && analysesModel.visible
			...
			{
				id:						analysesForm
				visible:				splitViewContainer.hasAnalysis && analysesModel.visible
				width:					splitViewContainer.hasAnalysis ? implicitWidth : 0
				height:					parent.height
				x:						extraBorder.width
			}
		}
		...
		JASPSplitHandle
		{
			...
			visible:				splitViewContainer.hasAnalysis && !ribbonModel.dataMode
