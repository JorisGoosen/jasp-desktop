...
bool DynamicModules::initializeModule(Modules::DynamicModule * module)
{
	try
	{
		...
		if(!wasAddedAlready)	emit dynamicModuleAdded(module);
		else if(oldModule)
		{
...
QString DynamicModules::getDescriptionJsonFromArchive(QString archiveFilePath)
{
	...
	//The javascript toJSON is more picky then JsonCPP so we do something seemingly crazy here, transforming the text into json and back to text, but this removes "comments" (they are not officially allowed in JSON but jsoncpp does not care and javascript crashes on it)
