...
void DynamicModules::installationPackagesFailed(const QString & moduleName, const QString & errorMessage)
{
	...
	MessageForwarder::showWarning(tr("Installation of Module %1 failed").arg(moduleName), tr("The installation of Module %1 failed with the following errormessage:\n'%2'").arg(moduleName).arg(errorMessage));
...
void DynamicModules::devModCopyDescription()
{
	...
	{
		if(dst.exists())
			MessageForwarder::showWarning(tr("Missing %1").arg(descJson), tr("You seem to have removed %1 from your development module directory. Without it your module cannot work, make sure to put it back. For now your old %1 file will be kept.").arg(descJson));
		else
		{
