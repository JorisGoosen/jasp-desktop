...
void MainWindow::_analysisSaveImageHandler(Analysis* analysis, QString options)
{
	...
	QString finalPath = MessageForwarder::browseSaveFile(tr("Save JASP Image"), "",
		 "Portable Network Graphics (*.png);;Portable Document Format (*.pdf);;Encapsulated PostScript (*.eps);;300 dpi Tagged Image File (*.tiff)", //Don't translate these as that will break the code a bit down
		 &selectedFilter);
	...
	if (!finalPath.isEmpty())
	{
		...
		//maybe this code should first check the extension of the entered filepath? If there is none we can use selectedFilter otherwise it is probably better to take whatever the user typed.
...
void MainWindow::dataSetIORequestHandler(FileEvent *event)
{
	...
	else if (event->operation() == FileEvent::FileClose)
	{
		if (_package->isModified())
		{
			...
			switch(MessageForwarder::showSaveDiscardCancel(tr("Save Workspace?"), tr("Save changes to workspace %s before closing?\n\nYour changes will be lost if you don't save them.").arg(title)))
			{
			default:
...
bool MainWindow::checkPackageModifiedBeforeClosing()
{
	...
	switch(MessageForwarder::showSaveDiscardCancel(tr("Workspace has changes"), tr("Save changes to workspace %s before closing?\n\nYour changes will be lost if you don't save them.").arg(title)))
	{
	case MessageForwarder::DialogResponse::Save:
	{
...
void MainWindow::dataSetIOCompleted(FileEvent *event)
{
	...
	else if (event->operation() == FileEvent::FileSave)
	{
		...
		{
			MessageForwarder::showWarning(tr("Save failed"), tr("Unable to save file because:\n%s").arg(event->message()));
...
void MainWindow::openGitHubBugReport() const
{
	...
	//Seems a bit weird to translate this template?
...
void MainWindow::fatalError()
{
	...
	if (exiting == false)
	{
		...
		if(MessageForwarder::showYesNo(tr("Error"), tr("JASP has experienced an unexpected internal error:\n%s\n\n"
			"JASP cannot continue and will close.\n\nWe would be grateful if you could report this error to the JASP team.").arg(_fatalError), tr("Report", "Reporting a bug on github"), tr("Exit")))
		{
...
void MainWindow::startDataEditor(QString path)
{
	...
	if (!useDefaultSpreadsheetEditor)
	{
		...
		if (!QProcess::startDetached(startProcess))
			MessageForwarder::showWarning(tr("Start Spreadsheet Editor"), tr("Unable to start the (spreadsheet) editor : '%s'. Please check your editor settings in the preference menu.").arg(appname));
	}
	else
#endif
		if (!QDesktopServices::openUrl(QUrl("file:///" + path, QUrl::TolerantMode)))
			MessageForwarder::showWarning(tr("Start Spreadsheet Editor"), tr("No default spreadsheet editor for file %s. Use Preferences to set the right editor.").arg(fileInfo.completeBaseName()));
