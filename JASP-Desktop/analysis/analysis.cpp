...
Json::Value Analysis::editOptionsOfPlot(const std::string & uniqueName)
{
	...
	if(!_editOptionsOfPlot(_results, uniqueName, editOptions))
		MessageForwarder::showWarning(QString(tr("Could not find edit options of plot %s so plot editing will not work...")).arg(tq(uniqueName)));
...
void Analysis::setEditOptionsOfPlot(const std::string & uniqueName, const Json::Value & editOptions)
{
	if(!_setEditOptionsOfPlot(_results, uniqueName, editOptions))
		MessageForwarder::showWarning(QString(tr("Could not find set edit options of plot %s so plot editing will not remember anything (if it evens works)...")).arg(tq(uniqueName)));
}
